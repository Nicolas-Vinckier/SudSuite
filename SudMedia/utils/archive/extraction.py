"""Moteurs d'extraction ZIP et TAR.XZ avec contrôles de sécurité."""
...
from ..filesystem.paths import safe_member_path
    ...
    with zipfile.ZipFile(archive_path, "r", allowZip64=True) as archive:
        for info in archive.infolist():
            ...
            if unix_type == stat.S_IFLNK:
                raise ExtractionBackendError(f"Lien symbolique refusé par sécurité : {info.filename}")
            if not info.is_dir():
                files.append(info)
...
def extract_zip_python(archive_path, destination, file_infos, total_size, workers, quiet=False):
    ...
    progress = Progress(len(file_infos), total_size, label="Décompression", enabled=not quiet)
    progress.start()
...
def extract_zip_7zip(archive_path, destination, file_count, total_size, sevenzip, requested_threads, quiet=False):
    progress = Progress(file_count, total_size, label="Décompression", enabled=not quiet)
    command = [
...
def extract_tar_xz_7zip(archive_path, destination, sevenzip, requested_threads, quiet=False):
    ...
    if not quiet:
        print("[Décompression] Extraction TAR.XZ en flux avec 7-Zip...")
    ...
    if producer_code != 0 or consumer.returncode != 0:
        ...
        raise ExtractionBackendError(
            "Extraction TAR.XZ avec 7-Zip échouée" + (f" : {details[-2000:]}" if details else "")
        )
...
def extract_tar_members(archive, destination):
    ...
    for member in archive:
        ...
        if member.issym() or member.islnk():
            raise ExtractionBackendError(f"Lien refusé par sécurité dans l'archive : {member.name}")
        if member.isdir():
            target.mkdir(parents=True, exist_ok=True)
            continue
        if not member.isfile():
            raise ExtractionBackendError(f"Type d'entrée non pris en charge : {member.name}")
        target.parent.mkdir(parents=True, exist_ok=True)
...
def extract_tar_xz_external_xz(archive_path, destination, xz_path, requested_threads, quiet=False):
    Path(destination).mkdir(parents=True, exist_ok=False)
    if not quiet:
        print("[Décompression] XZ multithread + extraction TAR en flux...")
    env = os.environ.copy()
    ...
    try:
        ...
        if return_code != 0:
            ...
            raise ExtractionBackendError(
                f"Décompression XZ échouée (code {return_code})" + (f" : {details[-2000:]}" if details else "")
            )
        return file_count, total_size
    except BaseException:
...
def execute_extraction(archive_format, archive_path, destination, tools, engine, requested_threads, effective_threads, quiet=False):
    ...
    if engine == "external":
        raise RuntimeError("XZ ou 7-Zip est requis pour la décompression TAR.XZ externe.")
