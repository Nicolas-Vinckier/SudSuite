"""Vérification rapide ou complète des archives produites."""
...
def quick_verify_xz(output_path):
    if os.path.getsize(output_path) < len(XZ_MAGIC):
        raise RuntimeError("Archive TAR.XZ vide ou tronquée.")
...
def verify_archive(output_path, mode, verify_mode, expected_count, tools, requested_threads):
    if verify_mode == "none":
        print("[Vérification] Ignorée (--verify none) pour la vitesse maximale.")
        return
    start = time.perf_counter()
    print(f"[Vérification] Mode {verify_mode}...")
    if mode in {"1", "2"}:
    ...
    print(f"[Vérification] OK en {time.perf_counter() - start:.2f} s.")
